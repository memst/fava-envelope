...
class EvalFunction(EvalNode):
    ...
    def eval_args(self, context): # -> list[Any]:
        ...
...
class ResultSetEnvironment(CompilationEnvironment):
    ...
    """
    context_name = ...
    def get_column(self, name): # -> AttributeColumn:
...
def compile_expression(expr, environ): # -> EvalConstant:
...
def is_hashable_type(node): # -> bool:
...
def find_unique_name(name, allocated_set): # -> str:
...
EvalTarget = ...
def compile_targets(targets, environ): # -> list[Any]:
...
def compile_group_by(group_by, c_targets, environ): # -> tuple[Any, list[int] | None]:
...
def compile_order_by(order_by, c_targets, environ): # -> tuple[Any, list[Any]]:
...
EvalFrom = ...
def compile_from(from_clause, environ): # -> EvalFrom | None:
...
EvalQuery = ...
def compile_select(select, targets_environ, postings_environ, entries_environ): # -> EvalQuery:
...
def transform_journal(journal): # -> Select:
...
def transform_balances(balances): # -> Select:
...
EvalPrint = ...
def compile_print(print_stmt, env_entries): # -> EvalPrint:
...
def compile(statement, targets_environ, postings_environ, entries_environ): # -> EvalQuery | EvalPrint:
